...
/** Decode one wire upload while rejecting non-canonical base64 forms; the empty string is the canonical encoding of a zero-byte file. */
function decodeUpload(data) {
	const decoded = Buffer.from(data, "base64");
	if (decoded.toString("base64") !== data) throw new Error("knowledge upload is not canonical base64");
...
let KnowledgeController = (() => {
	...
	return class KnowledgeController extends _classSuper {
		static {
			...
			__esDecorate(this, null, _removeDocument_decorators, {
				kind: "method",
				name: "removeDocument",
				static: false,
				private: false,
				access: {
					has: (obj) => "removeDocument" in obj,
					get: (obj) => obj.removeDocument
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
		...
		provider() {
			...
			if (knowledge === void 0) throw new RemoteError("gateway/internal", "knowledge service is absent: this deployment does not mount a knowledge provider (e.g. @deepseek-ai/dsh-knowledge-store) in its composition", {});
			return knowledge;
		}
		/**
		...
		*/
		async list() {
			return (await this.provider().list()).map(entryView);
		}
		/**
		...
		*/
		async ingest(filename, data, title) {
			try {
				...
				return {
					ingested: result.ingested.map(entryView),
...
//#endregion
export { KnowledgeController, KnowledgeController as default };
