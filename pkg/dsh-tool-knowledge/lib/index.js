...
const inject = ["tools", "knowledge"];
/** Schemastery validation for {@link Config}. */
const Config = z.object({ maxResults: z.number().step(1).min(1).max(50).default(8) });
const DESCRIPTION = "Document library of project reference materials. Users upload documents (text, Markdown, HTML, PDF, Word); the tool extracts and indexes them. Actions: `ingest` one file or a directory of files by path (re-ingesting a path refreshes its content); `search` passages by meaning, returning the document title and filename for citation; `list` every document; `remove` one document by id. Use it when a task references uploaded materials or background documents.";
...
function apply(ctx, config) {
	...
	ctx.tools.register(defineTool({
		...
		execute(args) {
			switch (args.action) {
				case "ingest": return ingest(ctx, args);
				case "search": return search(ctx, args, maxResults);
				case "list": return list(ctx);
				case "remove": return remove(ctx, args);
			}
		},
		presentCall: (args) => ({
			card: "generic",
			title: `Knowledge ${args.action}`,
			kind: "other",
			rawInput: args
		})
	}));
...
async function ingest(ctx, args) {
	...
	if (path === void 0 || path.length === 0) throw new Error("knowledge ingest requires `path`");
	const result = await ctx.knowledge.ingest({
		path,
		...args.title !== void 0 && args.title.trim().length > 0 ? { title: args.title.trim() } : {}
	});
	return {
		action: "ingest",
...
async function search(ctx, args, maxResults) {
	...
	if (text === void 0 || text.length === 0) throw new Error("knowledge search requires non-empty `text`");
	return {
		action: "search",
		matches: (await ctx.knowledge.search({
			text,
			topK: maxResults
		})).map((match) => ({
			...
			score: match.score,
			...match.fusion === void 0 ? {} : { fusion: match.fusion }
		}))
	};
}
...
async function list(ctx) {
	return {
		action: "list",
		documents: (await ctx.knowledge.list()).map((entry) => ({
			id: entry.id,
			title: entry.title,
			filename: entry.filename,
			format: entry.format,
...
async function remove(ctx, args) {
	if (args.id === void 0 || args.id.length === 0) throw new Error("knowledge remove requires `id`");
	return {
		action: "remove",
		removed: await ctx.knowledge.remove(KnowledgeId(args.id))
	};
}
...
function renderResult(action, value) {
	switch (action) {
		case "ingest": {
			...
			if (skipped > 0 && value.skipped !== void 0) {
		...
		case "search": return value.matches === void 0 || value.matches.length === 0 ? "No matching passages." : `Found ${value.matches.length} matching passages.`;
		case "list": return value.documents === void 0 ? "No documents in the library." : `${value.documents.length} documents in the library.`;
		case "remove": return value.removed === true ? "Document removed." : "No document with that id.";
		default: return "Unknown knowledge action.";
	}
}
//#endregion
